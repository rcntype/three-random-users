//[pk] could make this terser by putting the API URL in a variable, and also by doing the .JSONing inside the promise.all
const randomPromise1 = fetch(
	"https://acme-users-api-rev.herokuapp.com/api/users/random"
).then(response => response.json())
const randomPromise2 = fetch(
	"https://acme-users-api-rev.herokuapp.com/api/users/random"
).then(response => response.json())
const randomPromise3 = fetch(
	"https://acme-users-api-rev.herokuapp.com/api/users/random"
).then(response => response.json())
...
window.addEventListener("hashchange", () => {
	...
	//[pk] we talked abt this in office hours...
})
